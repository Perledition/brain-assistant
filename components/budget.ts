 ...
 * @returns void
 */
export function writeFixPointToDisk(
    ...
    fs.writeFile(
        `${settings.VaultPath}/.obsidian/plugins/brain-assistant/log_data.json`,
 ...
 * @returns log_data
 */
async function loadSpendFromDisk(settings: BrainAssistantPluginSettings): Promise<BudgetLogData> {
    return new Promise((resolve, reject) => {
      fs.readFile(`${settings.VaultPath}/.obsidian/plugins/brain-assistant/log_data.json`, 'utf8', (err, data) => {
        if (err) {
          reject(err);
          return;
        }
 ...
 */
export async function estimateRemainingBudget(requestedLogs: any, settings: BrainAssistantPluginSettings): Promise<number> {
    
  // load spend tracking from disk
    let existingData = await loadSpendFromDisk(settings)
    ...
    // concat arrays
    let harmonizedLogs: AlephAlphaLog[] = requestedLogs.concat(existingData["logs"])
    ...
    writeSpendToDisk(existingData, settings);
    ...
    try {
      ...
      console.log("could not find credits")
    }
    const sum = spend.reduce((sum, value) => sum + value, 0);

    // substract from setting set budget
    const budget: number = parseFloat(settings.Budget.toString().replace(",", "."))
